...
function [] = training_testing()
    ...
    libsvm_folder = './libsvm-3.18/matlab/';
    model_fn = 'model.mat';
    ...
    for part = 1 : k
        ...
        % Train the SVM - Gaussian kernel (RBF = Radial Basis Function)
        ...
        model = svmtrain(labels_training, features_training, '-t 3 -c 10.0 -g 0.07 -b 1');
        save(model_fn, 'model');
        ...
        for sample = 1 : nTesting
            ...
            [predict, accuracy, prob] = svmpredict(label, feature, model, '-b 1');
